...
export default function ExpenseDashboard() {
  ...
  const uniqueCategories = ["All", ...new Set(filteredData.map(item => item.Category).filter(Boolean))];
  const uniqueModes = ["All", ...new Set(filteredData.map(item => item['Payment Mode']).filter(Boolean))];
  ...
  const getLedgerData = () => {
    return filteredData.filter(item => {
      ...
      const matchesSearch = 
        ...
        (item['Sub-Category'] && item['Sub-Category'].toLowerCase().includes(searchQuery.toLowerCase()));
      
      // 2. Filters
      const matchesType = typeFilter === "All" || item.Type === typeFilter;
      const matchesCategory = categoryFilter === "All" || item.Category === categoryFilter;
      const matchesMode = modeFilter === "All" || item['Payment Mode'] === modeFilter;

      // 3. Date Range
      let matchesDate = true;
      if (dateRange.start) {
        matchesDate = matchesDate && new Date(item.Date) >= new Date(dateRange.start);
      }
      if (dateRange.end) {
        matchesDate = matchesDate && new Date(item.Date) <= new Date(dateRange.end);
      }
      
      return matchesSearch && matchesType && matchesCategory && matchesMode && matchesDate;
    });
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
          ...
          <div className="bg-white/5 border border-white/10 backdrop-blur-md rounded-2xl p-6 hover:border-white/20 transition-all group">
            <div className="flex justify-between items-start mb-4">
              ...
              <span className="text-xs font-mono text-green-400 bg-green-500/10 px-2 py-1 rounded">+ Inflow</span>
            </div>
          ...
          <div className="bg-white/5 border border-white/10 backdrop-blur-md rounded-2xl p-6 hover:border-white/20 transition-all group">
            <div className="flex justify-between items-start mb-4">
              ...
              <span className="text-xs font-mono text-red-400 bg-red-500/10 px-2 py-1 rounded">- Outflow</span>
            </div>
          ...
          <div className="bg-gradient-to-br from-white/10 to-white/5 border border-white/20 backdrop-blur-md rounded-2xl p-6 relative overflow-hidden group">
            ...
            <div className="flex justify-between items-start mb-4 relative z-10">
              ...
              <span className={`text-xs font-mono px-2 py-1 rounded ${summary.balance >= 0 ? 'text-blue-400 bg-blue-500/10' : 'text-red-400 bg-red-500/10'}`}>
                Net Position
              </span>
            </div>
            <p className="text-sm text-gray-300 mb-1 relative z-10">Available Balance</p>
            <h2 className={`text-3xl font-bold font-mono tracking-tight relative z-10 ${summary.balance >= 0 ? 'text-white' : 'text-red-400'}`}>
          ...
          <div className="p-6 border-b border-white/5 space-y-6 bg-white/[0.02]">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              ...
              <div className="relative group w-full md:w-64">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-[#d6f928] transition-colors" size={14} />
                <input 
                  type="text" 
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              ...
              <div className="relative">
                 <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} className="w-full appearance-none bg-white/5 border border-white/10 rounded-lg py-2 pl-3 pr-8 text-xs text-gray-300 focus:outline-none focus:border-white/30 focus:text-white transition-colors cursor-pointer">
                   <option value="All">All Types</option>
                   <option value="Income">Income Only</option>
                   <option value="Expense">Expense Only</option>
                 </select>
                 <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-gray-500"><ChevronDown size={12} /></div>
              </div>

              {/* Category Filter */}
              <div className="relative">
                 <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} className="w-full appearance-none bg-white/5 border border-white/10 rounded-lg py-2 pl-3 pr-8 text-xs text-gray-300 focus:outline-none focus:border-white/30 focus:text-white transition-colors cursor-pointer">
                   <option value="All">All Categories</option>
                   {uniqueCategories.map(c => c !== "All" && <option key={c} value={c}>{c}</option>)}
                 </select>
                 <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-gray-500"><ChevronDown size={12} /></div>
              </div>
              ...
              {/* Payment Mode Filter */}
              <div className="relative">
                 <select value={modeFilter} onChange={(e) => setModeFilter(e.target.value)} className="w-full appearance-none bg-white/5 border border-white/10 rounded-lg py-2 pl-3 pr-8 text-xs text-gray-300 focus:outline-none focus:border-white/30 focus:text-white transition-colors cursor-pointer">
                   <option value="All">All Modes</option>
                   {uniqueModes.map(m => m !== "All" && <option key={m} value={m}>{m}</option>)}
                 </select>
                 <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-gray-500"><ChevronDown size={12} /></div>
              </div>
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-white/5">
                ...
                {ledgerTransactions.length === 0 && (
                  <tr><td colSpan="3" className="px-6 py-16 text-center text-gray-600 italic">No transactions found matching your filters.</td></tr>
                )}
              </tbody>
            </table>
          </div>
